...
def _load_short_audio_chunks() -> list:
    """Load en_short.wav and split into 512-sample float32 chunks.

    en_short.wav contains exactly one utterance followed by silence,
    guaranteed to produce exactly one final recognition result.

    Returns:
        List of numpy float32 arrays, each of length _CHUNK_SIZE.
    """
    return _load_wav_chunks(_EN_SHORT_WAV)


def _load_wav_chunks(path: Path) -> list:
    """Load a WAV file and split into 512-sample float32 chunks.
...
class TestSingleClientFinalization:
    """Full pipeline with en_short.wav (single utterance) produces exactly one finalized result."""

    def test_single_client_receives_finalization(self) -> None:
        server = _make_server()
        try:
            audio_chunks = _load_short_audio_chunks()
    ...
    def test_single_client_finalization_text_is_nonempty(self) -> None:
        ...
        try:
            ...
            assert final_frames[0].get("text") == "one two three"
        finally:
            server.stop()
...
class TestDualClientIsolation:
    ...
    def test_two_sessions_each_receive_finalization(self) -> None:
        ...
        assert _count_finals(frames_a) == 1, "Session A must receive exactly one finalization"
        assert _count_finals(frames_b) == 1, "Session B must receive exactly one finalization"
...
class TestShutdownServerFirst:
    ...
    def test_server_shutdown_closes_connection_and_leaves_valid_state(self) -> None:
        """When server stops, websocket becomes CLOSED and client app_state is valid.
...
class TestGracefulDrain:
    ...
    def test_no_threads_remain_after_stop(self) -> None:
        server = _make_server()
        ctx = _connect_client_transport(f"ws://127.0.0.1:{server.port}")
